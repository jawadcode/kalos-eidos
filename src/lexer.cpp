...
auto Lexer::peek_token() -> TokenKind {
    if (this->peeked.has_value()) return this->peeked.value().data;
    else {
...
static inline auto is_ident(char c) -> bool { return std::isalpha(c) || c == '_'; }

auto Lexer::next_kind() -> TokenKind {
    if (this->is_at_end()) return TokenKind::TOK_EOF;

    auto ch = this->next_char();
    if (is_ident(ch)) return this->ident();
    else if (std::isdigit(ch)) return this->number();
    else switch (ch) {
        ...
        case '<':
            ...
            } else return TokenKind::TOK_LT;
        case '>':
            if (this->peek_char() == '=') {
                this->skip_char();
                return TokenKind::TOK_GEQ;
            } else return TokenKind::TOK_GT;
        case '=':
            if (this->peek_char() == '=') {
                this->skip_char();
                return TokenKind::TOK_EQ;
            } else return TokenKind::TOK_ASSIGN;
        case '!':
            if (this->peek_char() == '=') {
                this->skip_char();
                return TokenKind::TOK_NEQ;
            } else return TokenKind::TOK_ERR;
        default: return TokenKind::TOK_ERR;
        }
}

auto Lexer::ident() -> TokenKind {
    while (is_ident(this->peek_char()) || std::isdigit(this->peek_char())) this->skip_char();

    auto ident_source = this->source.substr(this->start, this->current - this->start);
...
auto Lexer::number() -> TokenKind {
    while (std::isdigit(this->peek_char())) this->skip_char();
...
inline auto Lexer::is_at_end() const -> bool { return this->current == this->source.length(); }
...
inline auto Lexer::skip_whitespace() -> void {
    while (true) {
        ...
        if (std::isspace(peeked)) {
            ...
            while (this->peek_char() != '\n' && this->peek_char() != '\0') this->skip_char();
        } else break;
    }
}
